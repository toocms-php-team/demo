...
class Page{
    ...
    private $config  = array(
        'header' => '<li class="disabled"><a class="javascript:void(0)">共 %TOTAL_ROW% 条记录</a></li>',
        'prev'   => 'Prev',
        'next'   => 'Next',
        'first'  => '1...',
        'last'   => '...%TOTAL_PAGE%',
    ...
    public function show() {
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page > 0 && $this->totalPages != 1){
                    $link_page .= '<li class="active"><a class="javascript:void(0)">' . $page . '</a></li>';
                }
            }
        }
        ...
        return !empty($page_str) && MODULE_NAME == 'Bms' ? "<ul>{$page_str} {$this->_manualPage()}</ul>" : "<ul>{$page_str}</ul>";
    }
     ...
     * 手动输入页号跳转
     */
    private function _manualPage() {
        $html = '<li>
                    <a style="padding: 0 !important"><input type="text" id="page_num" style="width:35px;height:16px;margin-bottom: 0;border: none"></a>
                </li>
                <li>
                    <a class="num" href="javascript:void(0)" id="go" data-url="'.Cookie('__forward__').'" data-p="'.intval($_REQUEST[$this->p]).'" data-suffix="'.C('URL_HTML_SUFFIX').'">GO</a>
                </li>';
        return $html;
    }
}
