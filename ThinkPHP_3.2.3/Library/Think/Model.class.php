...
class Model {
    ...
    public function getField($field,$sepa=null) {
        ...
        if(strpos($field,',') && false !== $sepa) { // 多字段
            ...
            // 返回数据个数
            //if(true !== $sepa) {
            //黑暗中的武者 修改于 2015-11-7  在大数据量下 执行 条件语句+count的时候 sql语句后面加上了 （limit 1）使count语句耗时很大 特此执行count时去掉limit 1
            //经测试 在100万数据量下 count语句加上（limit 1）执行时间为 8-9s  去掉后为 1s左右
            if(true !== $sepa && false === strpos($field,'COUNT')) {// 当sepa指定为true的时候 返回所有数据
            ...
            if(!empty($result)) {
                ...
                //黑暗中的武者 修改于 2015-11-7  在大数据量下 执行 条件语句+count的时候 sql语句后面加上了 limit 1使count语句耗时很大
                if(true !== $sepa && (1==$options['limit'] || false !== strpos($field,'COUNT'))) {
                    $data   =   reset($result[0]);
                    if(isset($cache)){
                        S($key,$data,$cache);
                    }            
