...
class Upload {
    ...
    private $config = array(
        ...
        'isThumb'           =>  false,    // 是否对上传图片进行缩略图处理
        'thumbMaxWidth'     =>  '', // 缩略图最大宽度  可设置多个逗号隔开  例：'100,100'
        'thumbMaxHeight'    =>  '', // 缩略图最大高度  可设置多个逗号隔开  例：'100,100'
        'thumbPrefix'       =>  'thumb_',// 缩略图前缀   可设置多个逗号隔开  例：'thumb_,thumb_1_'
        'thumbSuffix'       =>  '', //缩略图后缀  可设置多个逗号隔开  例：'_1,_2'
        'isWater'           =>  false,
    );
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            /* 保存文件 并记录保存成功的文件 */
            if ($this->uploader->save($file,$this->replace,$this->config)) {
                unset($file['error'], $file['tmp_name']);
                if($this->isThumb) {
                    $file['thumb_prefix'] = $this->thumbPrefix;
                    $file['thumb_suffix'] = $this->thumbSuffix;
                }
                $info[$key] = $file;
            } else {
                $this->error = $this->uploader->getError();
