 ...
 * 插件 逻辑层
 */
class PluginsLogic extends BmsBaseLogic {
    ...
    function getList($request = array()) {
        ...
        if($dirs === false || !file_exists(PLUGIN_PATH)) {
            $this->setLogicInfo('插件目录不可读或者不存在！');
            return false;
        }
        //存储所有插件数组
        $plugins = array();
    ...
    function install($request = array()) {
        ...
        //判断插件类名是否存在
        if(!class_exists($class)) {
            $this->setLogicInfo('插件不存在！'); return false;
        }
        //插件存在 初始化插件类
        $PluginClass = new $class;
        //获取插件的信息 作者、版本等
        ...
        if(!$info || !$PluginClass->checkInfo()) {
            $this->setLogicInfo('插件信息缺失！'); return false;
        }
        session('plugins_install_error',null);
        //执行插件中的 预安装操作
        $install_flag = $PluginClass->install();
        if(!$install_flag) {
            $this->setLogicInfo('执行插件预安装操作失败'.session('plugins_install_error')); return false;
        }
        //初始化插件模型
        ...
        if(!$data) {
             $this->setLogicInfo($PluginsModel->getError()); return false;
        }
        //插件数据写入数据库
        if($PluginsModel->add($data)) {
            ...
            if($hooks_update) {
                //修改成功后 清空钩子缓存
                S('Hooks_Cache', null);
                $this->setLogicInfo('安装成功'); return true;
            } else {
                //修改失败 删除插件记录
                $PluginsModel->where(array('name'=>$plugin_name))->delete();
                $this->setLogicInfo('更新钩子处插件失败,请卸载后尝试重新安装'); return false;
            }
        } else {
            $this->setLogicInfo('写入插件数据失败'); return false;
        }
    }
    ...
    function uninstall($request = array()) {
        ...
        if(!$uninstall_flag) {
            $this->setLogicInfo('执行插件预卸载操作失败' . session('plugins_uninstall_error'));
            return false;
        }
        //移除钩子对应的该插件信息
        $hooks_update   =   D('Hooks','Logic')->removeHooks($plugin['name']);
        if($hooks_update === false) {
            $this->setLogicInfo('卸载插件所挂载的钩子数据失败'); return false;
        }
        //清空钩子缓存
        S('Hooks_Cache', null);
        //删除插件记录
        ...
        if($delete === false) {
            $this->setLogicInfo('卸载插件失败'); return false;
        } else {
            $this->setLogicInfo('卸载成功'); return true;
        }
    }
    ...
    public function config($request = array()) {
        ...
        if(!$plugin) {
            $this->setLogicInfo('插件未安装'); return false;
        }
        //获取插件类名
        $class = get_plugin_class($plugin['name']);
        //判断插件类名是否存在
            ...
            foreach ($plugin['config'] as $key => $value) {
                ...
                if($value['type'] != 'group') {
                    ...
                    if($plugin['config'][$key]['type'] == 'picture_union') {
                        $plugin['config'][$key]['value'] = api('File/getFiles',array($db_config[$key]));
                    }
                } else {
                    //存在分组配置
                    foreach ($value['options'] as $group => $options) {
                        foreach ($options['options'] as $g_key => $value) {
                            ...
                            if($plugin['config'][$key]['options'][$group]['options'][$g_key]['type'] == 'picture_union') {
                                $plugin['config'][$key]['options'][$group]['options'][$g_key]['value'] = api('File/getFiles',array($db_config[$g_key]));
                            }
                        }
                    }
                }
