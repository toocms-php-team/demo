        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <button class="btn btn-success href" url="<?php echo U('Article/add');?>" target-form="ids">新增</button>&nbsp;
                    <button class="btn btn-inverse ajax-post" url="<?php echo U('Article/forbidResume',array('model'=>'Article','value'=>0));?>" target-form="ids">禁用</button>&nbsp;
                    <button class="btn btn-info ajax-post" url="<?php echo U('Article/forbidResume',array('model'=>'Article','value'=>1));?>" target-form="ids">启用</button>&nbsp;
                    <button class="btn btn-danger confirm ajax-post" url="<?php echo U('Article/delete',array('model'=>'Article'));?>" target-form="ids">删除</button>&nbsp;
                    <button type="button" class="btn btn-primary move">移动</button>
                    <span style="display: none">
                    <?php echo (replace($select,'target-form','ids')); ?><button class="btn btn-primary ajax-post" url="<?php echo U('Article/move');?>" target-form="ids">确定</button>
                    </span>
                    <form action="<?php echo U('Article/index');?>" method="post" class="form">
                        <input type="text" name="title" value="<?php echo ($_REQUEST['title']); ?>" placeholder="文章标题">
                        <?php echo ($select); ?>
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped with-check reset-checkbox">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td><?php echo ((isset($row['cate_name']) && ($row['cate_name'] !== ""))?($row['cate_name']):'不存在或以删除'); if(($row['cate_status']) == "0"): ?>（分类已禁用）<?php endif; ?></td>
                                <td class="quick-edit" data-model="Article" data-id="<?php echo ($row['id']); ?>" data-field="view"><?php echo ($row['view']); ?></td>
                                <td class="quick-edit" data-model="Article" data-id="<?php echo ($row['id']); ?>" data-field="collections"><?php echo ($row['collections']); ?></td>
                                <td class="quick-edit" data-model="Article" data-id="<?php echo ($row['id']); ?>" data-field="sort"><?php echo ($row['sort']); ?></td>
                                <td>
                                    <select class="quick-edit-select" data-model="Article" data-id="<?php echo ($row['id']); ?>" data-field="is_best" data-focus-value="<?php echo ($row['is_best']); ?>" onchange="quick_edit_select_change(this)">
                                ...
                                <td>
                                    <a href="<?php echo U('Article/update',array('id'=>$row['id']));?>" title="编辑" class="tip-bottom">
                                        <span class="label label-success label-success-hover">编辑</span></a>&nbsp;
                                    <?php if($row['status'] == 0 or $row['status'] == 1): ?><a href="<?php echo U('Article/forbidResume',array('model'=>'Article','ids'=>$row['id'],'value'=>abs(1-$row['status'])));?>" title="<?php echo (show_status_name($row['status'])); ?>" class="tip-bottom ajax-get">
                                            <span class="label <?php echo (show_status_label($row['status'])); ?>"><?php echo (show_status_name($row['status'])); ?></span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('Article/delete',array('model'=>'Article','ids'=>$row['id']));?>" data-ing-html="正在删除 <i class='fa fa-spinner fa-spin'></i>" title="删除" class="tip-bottom confirm ajax-get">
