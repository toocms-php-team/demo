        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <button class="btn btn-success href" url="<?php echo U('SendTemplate/add');?>" target-form="ids">新增</button>&nbsp;
                    <button class="btn btn-inverse ajax-post" url="<?php echo U('SendTemplate/forbidResume',array('model'=>'SendTemplate','field'=>'status','value'=>0));?>" target-form="ids">禁用</button>&nbsp;
                    <button class="btn btn-info ajax-post" url="<?php echo U('SendTemplate/forbidResume',array('model'=>'SendTemplate','field'=>'status','value'=>1));?>" target-form="ids">启用</button>&nbsp;
                    <button class="btn btn-danger confirm ajax-post" url="<?php echo U('SendTemplate/delete',array('model'=>'SendTemplate'));?>" target-form="ids">删除</button>&nbsp;
                </div>
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped with-check reset-checkbox">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td><?php echo ($row['unique_code']); ?> <span class="badge badge-warning"><?php echo (get_send_cate($row['cate'])); ?></span></td>
                                <td><i class="fa <?php echo (get_send_icon($row['type'])); ?>"></i> <?php echo (get_config_title($row['type'],SEND_TEMPLATE_TYPES)); ?></td>
                                <td><?php echo ((isset($row['description']) && ($row['description'] !== ""))?($row['description']):'---'); ?></td>
                                <td><?php echo (get_status_title($row['status'])); ?></td>
                                <td>
                                    <a href="<?php echo U('SendTemplate/update',array('id'=>$row['id'],'model'=>'SendTemplate'));?>" title="编辑" class="">
                                        <span class="label label-success label-success-hover">编辑</span></a>&nbsp;
                                    <?php if($row['status'] == 0 or $row['status'] == 1): ?><a href="<?php echo U('SendTemplate/forbidResume',array('model'=>'SendTemplate','ids'=>$row['id'],'field'=>'status','value'=>abs(1-$row['status']),'cate'=>$row['cate']));?>" title="<?php echo (show_status_name($row['status'])); ?>" class="ajax-get">
                                            <span class="label <?php echo (show_status_label($row['status'])); ?>"><?php echo (show_status_name($row['status'])); ?></span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('SendTemplate/delete',array('model'=>'SendTemplate','ids'=>$row['id'],'cate'=>$row['cate']));?>" title="删除" data-ing-html="正在删除 <i class='fa fa-spinner fa-spin'></i>" class="confirm ajax-get">
