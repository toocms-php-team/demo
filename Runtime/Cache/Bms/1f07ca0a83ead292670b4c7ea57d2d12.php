                    ...
                    <ul class="quick-actions-horizontal">
                        <li>
                            <a href="javascript:void(0)">
                                <i class="icon-tag"></i>
                                <span>用户已使用优惠券总额：￥<?php echo ((isset($result['total_2']) && ($result['total_2'] !== ""))?($result['total_2']):0); ?></span>
                                <span>用户未使用优惠券总额：￥<?php echo ((isset($result['total_1']) && ($result['total_1'] !== ""))?($result['total_1']):0); ?></span>
                            </a>
                        </li>
                    </ul>
                    <form action="<?php echo U('MemberCoupon/index');?>" method="post" class="form">
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li data-value="1" data-title="已使用" <?php if(($_REQUEST['trend']) == "1"): ?>class="selected"<?php endif; ?>><a href="javascript:void(0)">已使用</a></li>
                                <li data-value="0" data-title="未使用" <?php if(($_REQUEST['trend']) == "0"): ?>class="selected"<?php endif; ?>><a href="javascript:void(0)">未使用</a></li>
                            </ul>
                        </div>
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped reset-checkbox">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td>
                                    面值：￥<?php echo ($row['face_value']); ?><br>
                                    满 ￥<?php echo ($row['use_condition']); ?> 可用
                                </td>
                                <td>
                                    生效时间：<?php echo (timestamp2date($row['start_use_time'])); ?><br>
                                    过期时间：<?php echo (timestamp2date($row['end_use_time'])); ?>
                                </td>
                                <td>
                                    <?php if($row['status'] == 1): ?>使用时间：<?php echo (timestamp2date($row['use_time'])); ?><br>
                                    订单号：<a href=""><?php echo ($row['order_sn']); ?></a>
                                        <?php else: ?>
                                        ---<?php endif; ?>
                                </td>
                                <td>
                                    <?php if($row['status'] == 1): ?><span class="yellow-1">已使用</span>
                                        <?php else: ?>
                                        <?php if($row['end_use_time'] < time()): ?><span class="red-1">已过期</span>
                                            <?php else: ?>
                                            未使用<?php endif; endif; ?>
                                </td>
                        ...
                        <tr>
                            <td colspan="2"></td>
                            <td colspan="10">
                                <span class="yellow-1">当前条件用户已使用优惠券总额：￥<?php echo ((isset($result['now_total_2']) && ($result['now_total_2'] !== ""))?($result['now_total_2']):0); ?><br>
                                当前条件用户未使用优惠券总额：￥<?php echo ((isset($result['now_total_1']) && ($result['now_total_1'] !== ""))?($result['now_total_1']):0); ?></span>
                            </td>
                        </tr>
                        </tbody>
                    </table>
