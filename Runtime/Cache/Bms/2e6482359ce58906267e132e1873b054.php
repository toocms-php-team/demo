        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <button class="btn btn-success href" url="<?php echo U('Goods/add');?>" target-form="ids">新增</button>&nbsp;
                    <button class="btn btn-inverse ajax-post" url="<?php echo U('Goods/forbidResume',array('model'=>'Goods','field'=>'status','value'=>0));?>" target-form="ids">禁用</button>&nbsp;
                    <button class="btn btn-info ajax-post" url="<?php echo U('Goods/forbidResume',array('model'=>'Goods','field'=>'status','value'=>1));?>" target-form="ids">启用</button>&nbsp;
                    <button class="btn btn-danger confirm ajax-post" url="<?php echo U('Goods/delete',array('model'=>'Goods'));?>" target-form="ids">删除</button>&nbsp;

                    <form action="<?php echo U('Goods/index');?>" method="post" class="form">
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped with-check reset-checkbox">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td><?php echo (msubstr($row['goods_name'],0,20)); ?></td>
                                <td><?php echo ($row['goods_cate_name']); ?></td>
                                <td>￥ <?php echo ($row['price']); ?> 元</td>
                                <td class="quick-edit" data-model="Goods" data-id="<?php echo ($row['id']); ?>" data-field="stock"><?php echo ($row['stock']); ?></td>
                                <!--<td><?php echo ($row['is_on_sale']); ?></td>-->
                                <td>
                                    <select class="quick-edit-select" data-model="Goods" data-id="<?php echo ($row['id']); ?>" data-field="is_integral" data-focus-value="<?php echo ($row['is_integral']); ?>" onchange="quick_edit_select_change(this)">
                                        <option value="1" >可用</option>
                                        <option value="0" >不可用</option>
                                    </select>
                                </td>
                                <!--<td>
                                    <select class="quick-edit-select" data-model="Goods" data-id="<?php echo ($row['id']); ?>" data-field="is_best" data-focus-value="<?php echo ($row['is_best']); ?>" onchange="quick_edit_select_change(this)">
                                        <option value="1">推荐</option>
                                        <option value="0">不推荐</option>
                                    </select>
                                </td>-->
                                <td class="quick-edit" data-model="Goods" data-id="<?php echo ($row['id']); ?>" data-field="sort"><?php echo ($row['sort']); ?></td>
                                <td><?php echo (get_status_title($row['status'])); ?></td>
                                <td><?php echo (timestamp2date($row['update_time'])); ?></td>
                                <td>
                                    <?php if(in_array($row['type_id'],$spec_types)): ?><a href="<?php echo U('Goods/products',array('id'=>$row['id']));?>" title="货品列表" class="">
                                        <span class="label label-primary label-primary-hover">货品列表</span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('Goods/update',array('model'=>'Goods','id'=>$row['id']));?>" title="编辑" class="">
                                        <span class="label label-success label-success-hover">编辑</span></a>&nbsp;
                                    <?php if($row['status'] == 0 or $row['status'] == 1): ?><a href="<?php echo U('Goods/forbidResume',array('model'=>'Goods','ids'=>$row['id'],'field'=>'status','value'=>abs(1-$row['status'])));?>" title="<?php echo (show_status_name($row['status'])); ?>" class="ajax-get">
                                        <span class="label <?php echo (show_status_label($row['status'])); ?>"><?php echo (show_status_name($row['status'])); ?></span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('Goods/delete',array('model'=>'Goods','ids'=>$row['id']));?>" title="删除" class="confirm ajax-get">
