        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-content no-padding">
                    <form class="form-horizontal text-height-27-form" method="post" action="<?php echo U('Channel/update');?>">
                        <input type="hidden" name="model" value="Channel">
                        <?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><div class="control-group">
                            <label class="control-label"><?php echo ($row['name']); ?></label>
                            <div class="controls">
                                <!--<input type="text" name="unique_code" value="<?php echo ($row['unique_code']); ?>" class="text-width-20">-->
                                <?php echo hook('upload',array('unique_sign'=>'channel'.$row["id"].'','field_name'=>'icon[]','field_value'=>$row['icon'],'save_path'=>'Config'));?>
                                <span class="help-block">* 上传频道图标</span>
                            </div>
                        </div>
                            <input type="hidden" name="id[]" value="<?php echo ($row['id']); ?>"><?php endforeach; endif; else: echo "" ;endif; ?>
