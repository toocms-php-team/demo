        ...
        <div class="span12">
            <div class="widget-box">
                <div class="widget-title widget-title-search list-widget-title">
                    ...
                    <button class="btn btn-inverse ajax-post" url="<?php echo U('Post/forbidResume',array('model'=>'Post','field'=>'status','value'=>0));?>" target-form="ids">禁用</button>&nbsp;
                    <button class="btn btn-info ajax-post" url="<?php echo U('Post/forbidResume',array('model'=>'Post','field'=>'status','value'=>1));?>" target-form="ids">启用</button>&nbsp;
                    <form action="<?php echo U('Post/index');?>" method="post" class="form">
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped with-check reset-checkbox">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td>
                                    <?php if($row['post_type'] == 2): ?><span class="badge badge-warning">转发</span>
                                        转自帖子：ID <?php echo ($row['forward_post_id']); ?>
                                        <?php else: ?>
                                        <span class="badge badge-success">原创</span><?php endif; ?>
                                </td>
                                <td><?php echo ($row['forward_times']); ?> 次</td>
                                <td><a href="<?php echo U('PostComment/index',array('post_id'=>$row['id']));?>"><?php echo ($row['comment_times']); ?> 次</a></td>
                                <td><?php echo ($row['fabulous_times']); ?> 次</td>
                                <td><?php echo ($row['collection_times']); ?> 次</td>
                                ...
                                <td>
                                    ...
                                    <a href="<?php echo U('Post/update',array('id'=>$row['id']));?>" title="详情" class="">
                                        <span class="label label-hover">详情</span></a>&nbsp;
                                    <?php if($row['status'] == 0 or $row['status'] == 1): ?><a href="<?php echo U('Post/forbidResume',array('model'=>'Post','ids'=>$row['id'],'field'=>'status','value'=>abs(1-$row['status'])));?>" title="<?php echo (show_status_name($row['status'])); ?>" class="ajax-get">
                                            <span class="label <?php echo (show_status_label($row['status'])); ?>"><?php echo (show_status_name($row['status'])); ?></span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('Post/delete',array('model'=>'Post','ids'=>$row['id']));?>" title="删除" data-ing-html="正在删除 <i class='fa fa-spinner fa-spin'></i>" class="confirm ajax-get">
                                        <span class="label label-important label-important-hover">删除</span></a>&nbsp;
                                </td>
                            </tr>
                            <tr>
                                <td colspan="5">
                                    <?php echo (msubstr($row['content'],0,105)); ?>
                                </td>
