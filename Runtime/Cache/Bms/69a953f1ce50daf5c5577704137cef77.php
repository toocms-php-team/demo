        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <!--<button class="btn btn-success href" url="<?php echo U('Reservation/add');?>" target-form="ids">新增</button>&nbsp;-->
                    <!--<button class="btn btn-inverse ajax-post" url="<?php echo U('Reservation/forbidResume',array('model'=>'Reservation','field'=>'status','value'=>0));?>" target-form="ids">禁用</button>&nbsp;-->
                    <button class="btn btn-info ajax-post" url="<?php echo U('Reservation/forbidResume',array('model'=>'Reservation','field'=>'status','value'=>1));?>" target-form="ids">处理完成</button>&nbsp;
                    <button class="btn btn-danger confirm ajax-post" url="<?php echo U('Reservation/delete',array('model'=>'Reservation'));?>" target-form="ids">删除</button>&nbsp;

                    <form action="<?php echo U('Reservation/index');?>" method="post" class="form">
                        ...
                        <div class="btn-group">
                            <button type="button" class="btn checked" data-default="--预约类型--"></button>
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li data-value="1" data-title="家装预约" <?php if(($_REQUEST['type']) == "1"): ?>class="selected"<?php endif; ?>><a href="javascript:void(0)">家装预约</a></li>
                                <li data-value="2" data-title="维修预约" <?php if(($_REQUEST['type']) == "2"): ?>class="selected"<?php endif; ?>><a href="javascript:void(0)">维修预约</a></li>
                            </ul>
                        </div>
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped with-check reset-checkbox">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td>
                                    <?php echo ($row['id']); ?>--
                                    <?php if($row['type'] == 1): ?><span class="badge badge-warning">家装预约</span><?php endif; ?>
                                    <?php if($row['type'] == 2): ?><span class="badge badge-success">维修预约</span><?php endif; ?>
                                </td>
                                <td>姓名：<?php echo ($row['contacts']); ?><br>电话：<?php echo ($row['mobile']); ?></td>
                                <td><?php echo ($row['province_name']); echo ($row['city_name']); echo ($row['area_name']); echo ($row['address_detail']); ?></td>
                                <td>
                                    <?php if($row['type'] == 1): echo ($row['acreage']); endif; ?>
                                    <?php if($row['type'] == 2): echo ($row['instruction']); endif; ?>
                                </td>
                                <td><?php echo (timestamp2date($row['create_time'])); ?></td>
                                <td><?php echo (timestamp2date($row['finish_time'])); ?></td>
                                <td>
                                    <?php if($row['status'] == 1): ?><span class="green-1">已处理</span>
                                        <?php else: ?>
                                        未处理<?php endif; ?>
                                </td>
                                <td>
                                    <!--<a href="<?php echo U('Reservation/update',array('id'=>$row['id'],'model'=>'Reservation'));?>" title="编辑" class="">
                                        <span class="label label-success label-success-hover">编辑</span></a>&nbsp;-->
                                    <?php if($row['status'] == 0): ?><a href="<?php echo U('Reservation/forbidResume',array('model'=>'Reservation','ids'=>$row['id'],'field'=>'status','value'=>abs(1-$row['status'])));?>" title="<?php echo (show_status_name($row['status'])); ?>" class="ajax-get">
                                        <span class="label <?php echo (show_status_label($row['status'])); ?>">处理完成</span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('Reservation/delete',array('model'=>'Reservation','ids'=>$row['id']));?>" title="删除" class="confirm ajax-get">
