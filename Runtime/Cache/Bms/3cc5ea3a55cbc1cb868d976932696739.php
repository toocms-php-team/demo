        ...
        <div class="span12">
            <div class="widget-box">
               ...
               <div class="widget-title widget-title-search list-widget-title">
                   <button class="btn btn-danger confirm ajax-post" url="<?php echo U('SendLog/delete',array('model'=>'SendLog'));?>" target-form="ids">删除记录</button>&nbsp;
                   <form action="<?php echo U('SendLog/index');?>" method="post" class="form">
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped with-check reset-checkbox">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo (msubstr($row['receiver'],0,30)); ?></td>
                            <td><?php echo ((isset($row['unique_code']) && ($row['unique_code'] !== ""))?($row['unique_code']):'未使用模板'); ?></td>
                            <td><?php echo ((isset($row['account']) && ($row['account'] !== ""))?($row['account']):'系统'); ?></td>
                            <td><?php echo (date('Y-m-d H:i',$row['create_time'])); ?></td>
                            <td><?php echo (get_send_status($row['status'])); ?></td>
                            <td>
                                <a href="<?php echo U('SendLog/detail',array('id'=>$row['id'],'model'=>'SendLog'));?>" title="明细" class="">
                                    <span class="label label-info label-info-hover">明细</span></a>&nbsp;
                                <a href="<?php echo U('SendLog/delete',array('model'=>'SendLog','ids'=>$row['id']));?>" title="移除" class="confirm ajax-get">
