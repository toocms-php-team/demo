        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <button class="btn btn-success href" url="<?php echo U('Keeper/add');?>" target-form="ids">新增</button>&nbsp;
                    <button class="btn btn-inverse ajax-post" url="<?php echo U('Keeper/forbidResume',array('model'=>'Keeper','field'=>'status','value'=>0));?>" target-form="ids">禁用</button>&nbsp;
                    <button class="btn btn-info ajax-post" url="<?php echo U('Keeper/forbidResume',array('model'=>'Keeper','field'=>'status','value'=>1));?>" target-form="ids">启用</button>&nbsp;
                </div>
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped with-check reset-checkbox">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td>
                                    <a href="<?php echo U('Keeper/update',array('id'=>$row['id'],'model'=>'Keeper'));?>" title="编辑" class="">
                                        <span class="label label-success label-success-hover">编辑</span></a>&nbsp;
                                    <?php if($row['status'] == 0 or $row['status'] == 1): ?><a href="<?php echo U('Keeper/forbidResume',array('model'=>'Keeper','ids'=>$row['id'],'field'=>'status','value'=>abs(1-$row['status'])));?>" title="<?php echo (show_status_name($row['status'])); ?>" class="ajax-get">
                                        <span class="label <?php echo (show_status_label($row['status'])); ?>"><?php echo (show_status_name($row['status'])); ?></span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('Keeper/delete',array('model'=>'Keeper','ids'=>$row['id']));?>" title="删除" class="confirm ajax-get">
