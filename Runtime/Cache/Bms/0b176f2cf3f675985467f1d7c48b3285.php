        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <!--<div class="widget-title widget-title-search list-widget-title">
                    <form action="<?php echo U('Comment/index');?>" method="post" class="form">
                        <input type="text" name="m_id" value="<?php echo ($_REQUEST['m_id']); ?>" placeholder="会员ID">
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                <li data-value="1" data-title="已审核" <?php if(($_REQUEST['status']) == "1"): ?>class="selected"<?php endif; ?>>
                                <a href="javascript:void(0)">已审核</a>
                                </li>
                                <li data-value="0" data-title="未审核" <?php if(($_REQUEST['status']) == "0"): ?>class="selected"<?php endif; ?>>
                ...
                <div class="widget-content no-padding">
                    <ul class="recent-posts">
                        <?php if(empty($list)): ?><li>Aho！没有查询结果！！</li><?php endif; ?>
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><li>
                                <div class="article-post">
                                    <span class="user-info">
                                        记录ID: <?php echo ($row['id']); ?>,　
                                        联系方式：<?php echo ((isset($row['contact']) && ($row['contact'] !== ""))?($row['contact']):'---'); ?>,　
                                        反馈时间：<?php echo (date('Y-m-d H:i',$row['create_time'])); ?>,　
                                        IP: <?php echo (long2ip($row['ip'])); ?>,　
                                        <?php $__FOR_START_22638__=0;$__FOR_END_22638__=$row['start'];for($i=$__FOR_START_22638__;$i < $__FOR_END_22638__;$i+=1){ ?><i class="fa fa-star"></i><?php } ?>
                                    </span>
                                    <p>
                                        反馈内容：<a href="javascript:void(0)" style="cursor: default"><?php echo ($row['content']); ?></a>
                                    </p>
                                    <p>
                                         <span class="user-info">
                                             处理状态：<?php if($row['status'] == 0): ?>未处理<?php endif; if($row['status'] == 1): ?>已处理<?php endif; ?>　
                                             处理时间：<?php echo (timestamp2date($row['finish_time'])); ?>
                                         </span>
                                    </p>
                                    <?php if($row['status'] == 0): ?><a href="<?php echo U('Feedback/forbidResume',array('model'=>'Feedback','ids'=>$row['id'],'field'=>'status','value'=>abs(1-$row['status'])));?>" title="" class="ajax-get">
                                            <span class="label <?php echo (show_status_label($row['status'])); ?>">标记为已处理</span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('Feedback/delete',array('model'=>'Feedback','ids'=>$row['id']));?>" title="删除" class="confirm ajax-get">
