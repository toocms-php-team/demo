        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <button class="btn btn-success href" url="<?php echo U('Advert/add');?>" target-form="ids">新增</button>&nbsp;
                </div>
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td><?php echo (get_config_title($row['position'],ADVERT_POSITION)); ?></td>
                                <td><?php echo ((isset($row['description']) && ($row['description'] !== ""))?($row['description']):'---'); ?></td>
                                <td>
                                    <?php echo (date('Y-m-d',$row['start_time'])); ?> ~ <?php echo (date('Y-m-d',$row['end_time'])); ?>
                                    <?php if($row['start_time'] < time() and $row['end_time'] > time()): ?><span class="badge badge-success">投放中</span>
                                    <?php else: ?>
                                        <span class="badge badge-important">已过期</span><?php endif; ?>
                                </td>
                                <td><?php echo (get_config_title($row['target_rule'],TARGET_RULES)); ?></td>
                                <td class="quick-edit" data-model="Advert" data-id="<?php echo ($row['id']); ?>" data-field="sort"><?php echo ($row['sort']); ?></td>
                                <td><?php echo (get_status_title($row['status'])); ?></td>
                                <td>
                                    <a href="<?php echo U('Advert/update',array('id'=>$row['id']));?>" title="编辑" class="">
                                        <span class="label label-success label-success-hover">编辑</span></a>&nbsp;
                                    <?php if($row['status'] == 0 or $row['status'] == 1): ?><a href="<?php echo U('Advert/forbidResume',array('model'=>'Advert','ids'=>$row['id'],'field'=>'status','value'=>abs(1-$row['status'])));?>" title="<?php echo (show_status_name($row['status'])); ?>" class="ajax-get">
                                            <span class="label <?php echo (show_status_label($row['status'])); ?>"><?php echo (show_status_name($row['status'])); ?></span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('Advert/delete',array('model'=>'Advert','ids'=>$row['id']));?>" title="删除" class="confirm ajax-get">
