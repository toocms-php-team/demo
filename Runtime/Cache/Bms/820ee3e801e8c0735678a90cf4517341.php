        ...
        <div class="span12">
            <div class="widget-box">
                <ul class="nav nav-tabs">
                    <?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><li <?php if(($key) == $_REQUEST['config_group']): ?>class="active"<?php endif; ?>>
                        <a href="<?php echo U('ConfigSet/index',array('config_group'=>$key));?>" data-toggle=""><?php echo ($row); ?>配置</a>
                        </li><?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
                <!--<div class="widget-title"></div>-->
                <div class="widget-content tab-content">
                    <form class="form-horizontal text-height-27-form" method="post" action="<?php echo U('ConfigSet/update');?>" autocomplete="off">
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><div class="control-group">
                            <label class="control-label"><?php echo ($row['title']); ?></label>
                            <div class="controls">
                                <?php switch($row['type']): case "1": ?><input type="text" name="config[<?php echo ($row['name']); ?>]" value="<?php echo ($row['value']); ?>" class="number-only text-width-40">
                                        <span class="help-block"><?php echo ($row['remark']); ?></span><?php break;?>
                                    <?php case "2": ?><input type="text" name="config[<?php echo ($row['name']); ?>]" value="<?php echo ($row['value']); ?>" class="text-width-40">
                                        <span class="help-block"><?php echo ($row['remark']); ?></span><?php break;?>
                                    <?php case "3": ?><textarea name="config[<?php echo ($row['name']); ?>]" class="text-width-40"><?php echo ($row['value']); ?></textarea>
                                        <span class="help-block"><?php echo ($row['remark']); ?></span><?php break;?>
                                    <?php case "4": ?><textarea name="config[<?php echo ($row['name']); ?>]" class="text-width-40"><?php echo ($row['value']); ?></textarea>
                                        <span class="help-block"><?php echo ($row['remark']); ?></span><?php break;?>
                                    <?php case "5": ?><div class="btn-group">
                                            <button type="button" class="btn checked" data-default=""></button>
                                            <button class="btn dropdown-toggle" data-toggle="dropdown">
                                                <span class="caret"></span>
                                            </button>
                                            <ul class="dropdown-menu">
                                                <?php $_result=parse_config_attr($row['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ext): $mod = ($i % 2 );++$i;?><li data-value="<?php echo ($key); ?>" data-title="<?php echo ($ext); ?>" <?php if(($key) == $row['value']): ?>class="selected"<?php endif; ?>>
                                                        <a href="javascript:void(0)"><?php echo ($ext); ?></a>
                                                    </li><?php endforeach; endif; else: echo "" ;endif; ?>
                                            </ul>
                                        </div>
                                        <input type="hidden" name="config[<?php echo ($row['name']); ?>]" value="<?php echo ($row['value']); ?>">
                                        <span class="help-block"><?php echo ($row['remark']); ?></span><?php break;?>
                                    <?php case "6": echo hook('upload',array('unique_sign'=>$row['name'],'field_name'=>'config['.$row['name'].']','field_value'=>$row['value'],'save_path'=>'Config')); break; endswitch;?>
                            </div>
