                ...
                <div class="widget-content no-padding no-border">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                <td><?php echo (date('Ymd-His',$row['time'])); ?></td>
                                <td><?php echo ($row['compress']); ?></td>
                                <td><?php echo (format_bytes($row['size'])); ?></td>
                                <td><?php echo ($key); ?></td>
                                <td>
                                    <a href="<?php echo U('Database/doImport',array('time'=>$row['time']));?>" title="还原" data-ing-html="数据正在还原，请勿关闭此页面 <i class='fa fa-spinner fa-spin'></i>" class="ajax-get confirm">
                                        <span class="label label-info label-info-hover">还原</span></a>&nbsp;
                                    <a href="<?php echo U('Database/del',array('time'=>$row['time']));?>" title="删除" class="ajax-get confirm">
