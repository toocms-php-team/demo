        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <button class="btn btn-success href" url="<?php echo U('Coupon/add');?>" target-form="ids">新增</button>&nbsp;
                    <button class="btn btn-inverse ajax-post" url="<?php echo U('Coupon/forbidResume',array('model'=>'Coupon','field'=>'status','value'=>0));?>" target-form="ids">禁用</button>&nbsp;
                    <button class="btn btn-info ajax-post" url="<?php echo U('Coupon/forbidResume',array('model'=>'Coupon','field'=>'status','value'=>1));?>" target-form="ids">启用</button>&nbsp;
                    <button class="btn btn-danger confirm ajax-post" url="<?php echo U('Coupon/delete',array('model'=>'Coupon'));?>" target-form="ids">删除</button>&nbsp;
                </div>
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped with-check reset-checkbox">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td><?php echo ((isset($row['cate_name']) && ($row['cate_name'] !== ""))?($row['cate_name']):'全场通用'); ?></td>
                                <td>面值：￥ <?php echo ($row['face_value']); ?> 元 <br>可用条件：满￥ <?php echo ($row['use_condition']); ?> 元可用</td>
                                <td>开始使用时间：<?php echo (timestamp2date($row['start_use_time'])); ?> <br>有效期：<?php echo ($row['effective_time']); ?>天</td>
                                <td><?php echo (get_status_title($row['status'])); ?></td>
                                <td>
                                    <a href="<?php echo U('Coupon/update',array('id'=>$row['id'],'model'=>'Coupon'));?>" title="编辑" class="">
                                        <span class="label label-success label-success-hover">编辑</span></a>&nbsp;
                                    <?php if($row['status'] == 0 or $row['status'] == 1): ?><a href="<?php echo U('Coupon/forbidResume',array('model'=>'Coupon','ids'=>$row['id'],'field'=>'status','value'=>abs(1-$row['status'])));?>" title="<?php echo (show_status_name($row['status'])); ?>" class="ajax-get">
                                        <span class="label <?php echo (show_status_label($row['status'])); ?>"><?php echo (show_status_name($row['status'])); ?></span></a>&nbsp;<?php endif; ?>
                                    <a href="<?php echo U('Coupon/delete',array('model'=>'Coupon','ids'=>$row['id']));?>" title="删除" class="confirm ajax-get">
