        ...
        <div class="span12">
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <button class="btn btn-warning confirm ajax-post no-refresh" url="<?php echo U('Database/doExport');?>" target-form="tables" data-confirm="确认要进行数据备份吗？" data-ing-html="数据正在备份，请勿关闭此页面 <i class='fa fa-spinner fa-spin'></i>">确认备份</button>&nbsp;
                    <button class="btn btn-success ajax-post no-refresh" url="<?php echo U('Database/optimize');?>" target-form="tables" data-ing-html="数据表正在优化，请勿关闭此页面 <i class='fa fa-spinner fa-spin'></i>">优化表</button>&nbsp;
                    <button class="btn btn-info ajax-post no-refresh" url="<?php echo U('Database/repair');?>" target-form="tables" data-ing-html="数据表正在修复，请勿关闭此页面 <i class='fa fa-spinner fa-spin'></i>">修复表</button>&nbsp;
                </div>
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped with-check">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                <td><input type="checkbox" name="tables[]" value="<?php echo ($row['name']); ?>" class="tables" checked='checked'/></td>
                                <td><?php echo ($row['name']); ?></td>
                                <td><?php echo ((isset($row['rows']) && ($row['rows'] !== ""))?($row['rows']):'---'); ?></td>
                                <td><?php echo (format_bytes($row['data_length'])); ?></td>
                                <td><?php echo ((isset($row['comment']) && ($row['comment'] !== ""))?($row['comment']):'---'); ?></td>
                                <td><?php echo ((isset($row['create_time']) && ($row['create_time'] !== ""))?($row['create_time']):'---'); ?></td>
                                <td>
                                    <a href="<?php echo U('Database/optimize',array('tables'=>$row['name']));?>" title="优化表" data-ing-html="正在优化数据表 <i class='fa fa-spinner fa-spin'></i>" class="ajax-get no-refresh">
                                        <span class="label label-success label-success-hover">优化表</span></a>&nbsp;
                                    <a href="<?php echo U('Database/repair',array('tables'=>$row['name']));?>" title="修复表" data-ing-html="正在修复数据表 <i class='fa fa-spinner fa-spin'></i>" class="ajax-get no-refresh">
