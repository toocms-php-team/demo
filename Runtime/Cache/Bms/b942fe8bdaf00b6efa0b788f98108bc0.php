            ...
            <div class="widget-box">
                ...
                <div class="widget-title widget-title-search list-widget-title">
                    <ul class="quick-actions-horizontal">
                        <!--<li>
                            <a href="<?php echo U('Address/index',array('circle_id'=>0));?>">
                                <i class="icon-tag"></i>
                                <span>未分配商圈地址：<?php echo ($count); ?></span>
                            </a>
                        </li>-->
                    </ul>
                    <form action="<?php echo U('Address/index');?>" method="post" class="form">
                        <input type="text" name="m_id" value="<?php echo ($_REQUEST['m_id']); ?>" placeholder="用户ID">
                ...
                <div class="widget-content no-padding no-border">
                    <table class="table table-bordered table-striped">
                        <thead>
                        <tr>
                            <!--with-check reset-checkbox<th><input type="checkbox" id="title-checkbox" name="title-checkbox" class="check-all"/>--><!--<i class="icon-resize-vertical"></i>--></th>
                            <th>ID</th>
                            <th>账号</th>
                            <th>联系人</th>
                            <th>联系电话</th>
                        ...
                        <tbody class="tbody">
                        <?php if(empty($list)): ?><tr><td colspan="30"><?php echo L('_LIST_NO_RESULT_');?></td></tr><?php endif; ?>
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td><?php echo ($row['m_id']); ?>--<?php echo ($row['account']); ?></td>
                                <td><?php echo ($row['contacts']); ?></td>
                                <td><?php echo ($row['mobile']); ?></td>
                                <td><?php echo ($row['province_name']); echo ($row['city_name']); echo ($row['area_name']); echo ($row['address']); ?></td>
                                <td><?php echo (date('Y-m-d H:i',$row['create_time'])); ?></td>
                                <td>
                                    <a href="<?php echo U('Address/delete',array('model'=>'Address','ids'=>$row['id']));?>" title="删除" data-ing-html="" class="confirm ajax-get">
                                        <span class="label label-important label-important-hover">删除</span></a>&nbsp;
                                </td>
                                <td>
                                    <?php if($row['is_default'] == 1): ?><span class="badge badge-info">默认</span>
                                    <?php else: ?>
                                        ---<?php endif; ?>
                                </td>
